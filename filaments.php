...
if (!empty($_POST)) {
    ...
    $params = [$data['brand'], $data['name'], $data['type'], $data['color'], (int)$data['weight'], (float)$data['price']];
    if (isset($data['update_id'])) {
        $stmt = $pdo->prepare('UPDATE filaments SET brand = ?, name = ?, type = ?, color = ?, weight = ?, price = ? WHERE id = ?');
        $params[] = (int)$data['update_id'];
        $message = 'Filament succesvol bijgewerkt!';
    } else {
        $stmt = $pdo->prepare('INSERT INTO filaments (brand, name, type, color, weight, price) VALUES (?, ?, ?, ?, ?, ?)');
        $message = 'Filament succesvol toegevoegd!';
    }
    $stmt->execute($params);
    redirect_with_message("filaments.php?page=$page", $message);
}
?>
            ...
            <tbody>
                <?php foreach ($filaments as $filament): ?>
                    <tr class="filament-row" 
                        ...
                        <td><a href="delete.php?id=<?=$filament['id']?>&type=filament&page=<?=$page?>" class="delete-link">Verwijderen</a></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
